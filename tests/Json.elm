...
type alias Colouring =
  { colour : String
  }


type Msg =
  Rcvd (Result Error (Envelope Colouring))


receiveTest : Test
receiveTest =
  describe "receiveTest"
  [ test "Should turn an envelope with an encoded value into the right message" <|
    \_ ->
      let
        jsonEnv =
          Enc.object
        ...
        decoder =
          Dec.field "colour" Dec.string
          |> Dec.map Colouring
        recv = receive Rcvd decoder
      in
      case recv jsonEnv of
        Rcvd (Ok (Peer rec)) ->
          Expect.equal { colour = "Red" } rec.body
